...
tal_gwcsf - Computes fractional tissue component images from a T1 MRI
volume.
...
-z(dz)
   The output depth in bytes (Gout,Wout,Cout). Fout is always in float
   (-z4). The default is 4 bytes (float).
...
-N(tmp)
   Specify a basename to save the local 8bit histograms to disk as text
   files. Default:none.
...
-ri(x:y:z)
   Select the increment from one local fit region to the next region in
   pixels. Default: 10:10:10

-rt(x:y:z)
   Select the size of the region in pixels to compile final tissue
   fractions over. Default: 1:1:1

-rp(percent)
   Set the fraction of voxels in a local fit region (-rs) that must be
   inside the mask for the fit region to be considered valid. Default:
   0.10

-GXY(value)
   This is actually a collection of options. 'X' can be one of G, W or C
   and
...
-help
   This option will display a number of undocumented advanced options
   that can be set. It should be avoided in general, but is provided for
   the user with specific problems cases.
