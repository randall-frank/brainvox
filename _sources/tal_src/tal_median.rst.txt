...
This program applies a median smoothing filter to each voxel in a
volume. The filter can be of any rectangular size and is clipped at the
...
template for the output filtered volume.
...
-m(masktemp)
   This specifies a mask for the voxels to be smoothed. Only voxels
   which are inside the mask are used when computing any median.
...
-r(dx:dy:dz)
   ...
   For a 2D filter, specify one of the sizes to one. The default is:
   3:3:3.
