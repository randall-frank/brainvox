...

{
	...
	}
	{
		...
		if(stats->y_dim>1) y_dim1=(unsigned int)ceil((double)BIGY*stats->y_dim);
		else y_dim1=1;
	
		if(stats->z_dim>1) z_dim1=(unsigned int)ceil((double)BIGZ*stats->z_dim);
		else z_dim1=1;
		...
		{	
			double
				e00t=es[0][0],
				e01t=((double)BIGY)*es[0][1],
				e02t=((double)BIGZ)*es[0][2],
				e10t=es[1][0],
				e11t=((double)BIGY)*es[1][1],
				e12t=((double)BIGZ)*es[1][2],
				e20t=es[2][0],
				e21t=((double)BIGY)*es[2][1],
				e22t=((double)BIGZ)*es[2][2],
				e30t=es[3][0],
				e31t=((double)BIGY)*es[3][1],
				e32t=((double)BIGZ)*es[3][2];
		...
		/*Prepass interpolate along z */
		{
			...
			double w=2.0*pi/(stats->z_dim*(double)BIGZ);
			double a=0.0;
			unsigned int nfft=1;

			while(nfft<=stats->z_dim+z_dim1-1) nfft*=2;
			if(stats->z_dim>1 && (double)BIGZ!=1.0){
		...
		/* Now, for each plane, we prepass interpolate along y and then interpolate x and y */
		{
			...
			for(k=0;k<z_dim1;k++,x_k+=e20,x_k2+=e20i,y_k+=e21){
				...
				if(stats->y_dim>1 && (double)BIGY!=1.0){
	
					unsigned int oversamp=1;
					AIR_Boolean inited=FALSE;
					unsigned int ifac[20],ifac2[20];
					double w=2.0*pi/(stats->y_dim*(double)BIGY);
					double a=0.0;
					unsigned int nfft=1;
