...

{
	...
	}
	{
		...
		if(stats->z_dim>1) z_dim1=(unsigned int)ceil((double)BIGY*stats->z_dim);
		else z_dim1=1;	
		...
		/* Decompose the transformation matrix */
		{
			double 
				e00t=es[0][0],
				e01t=((double)BIGY)*es[0][2],
				e02t=es[0][1],
				e10t=es[2][0],
				e11t=((double)BIGY)*es[2][2],
				e12t=es[2][1],
				e20t=es[1][0],
				e21t=((double)BIGY)*es[1][2],
				e22t=es[1][1],
				e30t=es[3][0],
				e31t=((double)BIGY)*es[3][2],
				e32t=es[3][1];
		...
		/* Now, for each plane, we prepass interpolate along y and then interpolate x and y */
		{
			...
			for(k=0;k<y_dim1;k++,x_k+=e20,x_k2+=e20i,y_k+=e21){
				...
				/* Prepass interpolate along y*/
				{
					...
					double w=2.0*pi/(stats->z_dim*(double)BIGY);
					double a=0.0;
					...
					if(stats->z_dim>1 && (double)BIGY!=1.0){
