 ...
 *
 *  Note: This has been modified from the original GLUT sources
 *        to fix a couple of indexing modes and to avoid printing or
 *        calling exit(), by rjf, LLNL.
 *
...
_glmFindMaterial(GLMmodel* model, char* name)
{
  ...
  DBGPRINTF("_glmFindMaterial():  can't find material \"%s\".\n", name);
  i = 0;
...
_glmFirstPass(GLMmodel* model, FILE* file) 
{
  ...
  while(fscanf(file, "%s", buf) != EOF) {
	...
	break;
      default:
	DBGPRINTF("_glmFirstPass(): Unknown token \"%s\".\n", buf);
	return(-1);
	break;
      }
      break;
  ...
  DBGPRINTF(" Vertices: %d\n", numvertices);
  DBGPRINTF(" Normals: %d\n", numnormals);
  DBGPRINTF(" Texcoords: %d\n", numtexcoords);
  DBGPRINTF(" Triangles: %d\n", numtriangles);
  DBGPRINTF(" Groups: %d\n", model->numgroups);
...
_glmSecondPass(GLMmodel* model, FILE* file) 
{
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case 'f':				/* face */
      v = n = t = 0;
      fscanf(file, "%s", buf);
      /* can be one of %d, %d//%d, %d/%d, %d/%d/ %d/%d/%d %d//%d */
      if (strstr(buf, "//")) {
	/* v//n */
	sscanf(buf, "%d//%d", &v, &n);
	T(numtriangles).vindices[0] = R(v,numvertices);
	T(numtriangles).nindices[0] = R(n,numnormals);
	fscanf(file, "%d//%d", &v, &n);
	T(numtriangles).vindices[1] = R(v,numvertices);
	T(numtriangles).nindices[1] = R(n,numnormals);
	fscanf(file, "%d//%d", &v, &n);
	T(numtriangles).vindices[2] = R(v,numvertices);
	T(numtriangles).nindices[2] = R(n,numnormals);
	...
	/* v/t/n */
	T(numtriangles).vindices[0] = R(v,numvertices);
	T(numtriangles).tindices[0] = R(t,numtexcoords);
	T(numtriangles).nindices[0] = R(n,numnormals);
	fscanf(file, "%d/%d/%d", &v, &t, &n);
	T(numtriangles).vindices[1] = R(v,numvertices);
	T(numtriangles).tindices[1] = R(t,numtexcoords);
	T(numtriangles).nindices[1] = R(n,numnormals);
	fscanf(file, "%d/%d/%d", &v, &t, &n);
	T(numtriangles).vindices[2] = R(v,numvertices);
	T(numtriangles).tindices[2] = R(t,numtexcoords);
	T(numtriangles).nindices[2] = R(n,numnormals);
	group->triangles[group->numtriangles++] = numtriangles;
	numtriangles++;
	while(fscanf(file, "%d/%d/%d", &v, &t, &n) > 0) {
...
glmVertexNormals(GLMmodel* model, GLfloat angle)
{
  ...
  for (i = 1; i <= model->numvertices; i++) {
    ...
    node = members[i];
    if (!node)
      DBGPRINTF("glmVertexNormals(): vertex w/o a triangle\n");
  ...
  DBGPRINTF("glmVertexNormals(): %d normals generated\n", model->numnormals);
}
...
glmLinearTexture(GLMmodel* model)
{
  ...
  DBGPRINTF("glmLinearTexture(): generated %d linear texture coordinates\n",
	  model->numtexcoords);
}
...
glmSpheremapTexture(GLMmodel* model)
{
  ...
  DBGPRINTF("glmSpheremapTexture(): generated %d spheremap texture coords\n",
	 model->numtexcoords);
}
...
glmWriteOBJ(GLMmodel* model, char* filename, GLuint mode)
{
  ...
  if (mode & GLM_FLAT && !model->facetnorms) {
    DBGPRINTF("glmWriteOBJ() warning: flat normal output requested "
	   "with no facet normals defined.\n");
    mode &= ~GLM_FLAT;
  }
  if (mode & GLM_SMOOTH && !model->normals) {
    DBGPRINTF("glmWriteOBJ() warning: smooth normal output requested "
	   "with no normals defined.\n");
    mode &= ~GLM_SMOOTH;
  }
  if (mode & GLM_TEXTURE && !model->texcoords) {
    DBGPRINTF("glmWriteOBJ() warning: texture coordinate output requested "
...
glmDraw(GLMmodel* model, GLuint mode)
{
  ...
  /* do a bit of warning */
  if (mode & GLM_FLAT && !model->facetnorms) {
    DBGPRINTF("glmDraw() warning: flat render mode requested "
	   "with no facet normals defined.\n");
    mode &= ~GLM_FLAT;
  }
  if (mode & GLM_SMOOTH && !model->normals) {
    DBGPRINTF("glmDraw() warning: smooth render mode requested "
	   "with no normals defined.\n");
    mode &= ~GLM_SMOOTH;
  }
  if (mode & GLM_TEXTURE && !model->texcoords) {
    DBGPRINTF("glmDraw() warning: texture render mode requested "
	   "with no texture coordinates defined.\n");
    mode &= ~GLM_TEXTURE;
  }
  if (mode & GLM_FLAT && mode & GLM_SMOOTH) {
  ...
  if (mode & GLM_COLOR && !model->materials) {
    DBGPRINTF("glmDraw() warning: color render mode requested "
	   "with no materials defined.\n");
    mode &= ~GLM_COLOR;
  }
  if (mode & GLM_MATERIAL && !model->materials) {
    DBGPRINTF("glmDraw() warning: material render mode requested "
	   "with no materials defined.\n");
    mode &= ~GLM_MATERIAL;
  }
  if (mode & GLM_COLOR && mode & GLM_MATERIAL) {
    DBGPRINTF("glmDraw() warning: color and material render mode requested "
...
glmWeld(GLMmodel* model, GLfloat epsilon)
{
  ...
  DBGPRINTF("glmWeld(): %d redundant vertices.\n", 
