...
/*
    ...
    PROTOTYPE		: vl_render_pixmap_(void *map,short *val,
			VLINT32 dx,VLINT32 dy,VLINT32 ox,VLINT32 oy,
			VLINT32 flag,VLINT32 color,vset *set);
...
{
	VLINT32	i,j,k,t,offset;
	VLINT16		zval;
	VLUINT32	b2,im_val,col,Icolor,RB,MSA,SA;
	VLUINT32 	*l_in = (VLUINT32 *)map;
	VLUCHAR		*c_in = (VLUCHAR *)map;
	VLUINT32	*image = set->pickbuffer;

/* check for valid output */
	if (set->image == 0L) return;
	if (set->zbuffer == 0L) return;
	if (set->pickbuffer == 0L ) return;
...
/* place it in the buffer */
					image[offset] = vl_polygon_globals.object_id; 
	...
	VLINT32 ox,VLINT32 oy,VLINT32 flag,VLINT32 color,vset *set)
{
	...
	VLUINT32 	*l_in = (VLUINT32 *)map;
	VLUCHAR		*c_in = (VLUCHAR *)map;
	VLUINT32	*image = (VLUINT32 *)set->image;
