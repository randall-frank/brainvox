...
void getznorm(long int x, long int y,short *zbuf, double *norm,
		long int dx,long int dy)
{
	long int px1,px2,py1,py2,pt;
	double l;
...
#define FRAC vl_light_globals.jitter_frac
	...
	if (FRAC > 0.0) {
	norm[0] = norm[0] + 
		(((double)(rand() & 0xff)/255.0) * FRAC) - (FRAC/2);
	norm[1] = norm[1] + 
		(((double)(rand() & 0xff)/255.0) * FRAC) - (FRAC/2);
	}
...
void recolor(double *norm,double inten,long int *image,short *zbuf,
		long int *out,long int dx,long int dy)
{
	register long int x,y; 
	double znorm[3];
	register double place;
	register long int R,B,G;
	register long int mult;
...
void vl_addhaze_(long int *image,short *zbuf,long int *out,
			long int dx,long int dy,double *w)
{
	long int y,x,k;
	double	mult;
	long int R,G,B;
...
*/
	tmp = (w[0] - w[1])/(double)(vl_light_globals.hz_near -
	...
	for(y=0; y<dy; y++) {
		...
		for(x=0; x<dx; x++) {
			if (zbuf[k] < (VL_OUTTHERE))  {
				...
				mult = (w[0] - (tmp*
				   (double)(vl_light_globals.hz_near-zbuf[k])));
...
/* rotate */
	...
	vl_light_globals.l_table = (long int *)malloc(sizeof(long int)*32768);
...
{
	register unsigned long int i,r,b,it;
/* index is: 0xxx xxyy yyyz zzzz */
/* from: xxxx x000, yyyy y000, zzzz z000 */
	if (vl_light_globals.l_table == 0L) return;
	i = ((unsigned long int)(dx & 0xf8) << 7) | 
		((unsigned long int)(dy & 0xf8) << 2) |
		((unsigned long int)(dz & 0xf8) >> 3);
#ifdef DEBUG
	printf("test: %d %d %d : %x\n",dx,dy,dz,i);
#endif
