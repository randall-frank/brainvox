...
short vl_chrtbl[0200][_strokemax][3] = {
/* c0 */ {{0}},
...
static void draw_string_vector(char *s,long int type,vl_text_attrib *a,vl_int_vertex *pnt,vset *set);

static void draw_string_vector(char *s,long int type,vl_text_attrib *a,vl_int_vertex *pnt,vset *set)
{
    register long int i,j;
    double	dx,dy,ddx,ddy,sa,ca;
    unsigned char *str;	
    vl_int_vertex	v1,v2;
                ...
                i=0;
                  while (j=vl_chrtbl[*str][i][0]) {
                        switch (j) {
                           case 3:
/* move */
					ddx = (double)(vl_chrtbl[*str][i][1])*dx;
					ddy = (double)(vl_chrtbl[*str][i][2])*dy;
...
/* draw the line from v2 to v1 */
	                                vl_p_rendline((type & VL_P_MODEMASK),
                                       		(type & VL_P_OPTSMASK),
                                        	&(v2),&(v1),set);
                              break;
                        }
...
{
	long int	dx,dy,x,y,flag,color,i;
	void		*data;
	unsigned char	*d;
	short		zval;

	if (text_proc == 0L) {
		draw_string_vector(s,type,a,pnt,set);
	} else {
/* get the pixmap */
		data = 0;
