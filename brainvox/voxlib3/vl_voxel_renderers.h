...
/* get the X,Y point */  \
        	...
        	vx[2] = vid[2] >> 13;  \
/* run through the buffer */  \
		ZCOLORIN(vx,Opacity[i2],RBlut[i2],Glut[i2],image,zbuf); \
        }; 
...
#define MIPOVER \
/* if it is visible plot it */  \
	if ((loc_rop = ropacity[i]) > 0) {  \
/* get the X,Y point */  \
        	vx[0] = vid[0] >> 16;  \
        	vx[1] = vid[1] >> 16;  \
        	vx[2] = vid[2] >> 13;  \
/* run through the buffer */  \
		ZCOLORMIP(vx,loc_rop,RBrlut[i],image,zbuf); \
        }; 
...
/* get the X,Y point */  \
        	...
        	vx[2] = vid[2] >> 13;  \
/* run through the buffer */  \
		ZCOLORMNIP(vx,loc_rop,RBrlut[i],Grlut[i],image,zbuf); \
        }; 
...
#define MIPOVER24 \
/* if it is visible plot it */  \
	loc_rop = Ropa[i]+Gopa[i2]+Bopa[i3]; \
	if (loc_rop > 0) {   \
/* get the X,Y point */  \
        	vx[0] = vid[0] >> 16;  \
        	vx[1] = vid[1] >> 16;  \
        	vx[2] = vid[2] >> 13;  \
/* run through the buffer */  \
		ZCOLORMIP24(vx,Rlut[i],Glut[i2],Blut[i3],image,zbuf); \
        }; 

#define YIQMIPOVER24 \
/* if it is visible plot it */  \
/* 77 + 150 + 29 = 256 */	\
	loc_rop = Ropa[((77*i)+(150*i2)+(29*i3)) >> 8]; \
	if (loc_rop > 0) {   			\
/* get the X,Y point */  \
        	vx[0] = vid[0] >> 16;  \
        	vx[1] = vid[1] >> 16;  \
        	vx[2] = vid[2] >> 13;  \
/* run through the buffer */  \
	ZCOLORMIP(vx,loc_rop,(Rlut[i]|Glut[i2]|Blut[i3]),image,zbuf); \
        }; 
...
/* get the X,Y point */  \
        	...
        	vx[2] = vid[2] >> 13;  \
/* run through the buffer */  \
                wi = (Rlut[i] | Blut[i3])*loc_rop; \
                wj = (Glut[i2])*loc_rop; \
		ZCOLORIN(vx,255-loc_rop,wi,wj,image,zbuf); \
        }; 
...
/* get the X,Y point */  \
        	...
        	vx[2] = vid[2] >> 13;  \
/* run through the buffer */  \
ZCOLORIN24(vx,Ropa[i],Gopa[i2],Bopa[i3],Rlut[i],Glut[i2],Blut[i3],image,zbuf); \
        }; 
...
/* LIGHT the point */  \
                ...
                wz -= *(loc_dptr - mult[2]);  \
                wi = Rlut[i]; \
                vl_get_inten(wx,wy,wz,&wi,&loc_lop);  \
                wx = *(loc_d2ptr + mult[0]);  \
                wx -= *(loc_d2ptr - mult[0]);  \
                wy = *(loc_d2ptr + mult[1]);  \
                wy -= *(loc_d2ptr - mult[1]);  \
                ...
                wz -= *(loc_d2ptr - mult[2]);  \
                wj = Glut[i2]; \
                vl_get_inten(wx,wy,wz,&wj,&loc_lop);  \
                wx = *(loc_d3ptr + mult[0]);  \
                wx -= *(loc_d3ptr - mult[0]);  \
                wy = *(loc_d3ptr + mult[1]);  \
                wy -= *(loc_d3ptr - mult[1]);  \
                ...
                wz -= *(loc_d3ptr - mult[2]);  \
                wk = Blut[i3]; \
                vl_get_inten(wx,wy,wz,&wk,&loc_lop);  \
/* run through the buffer */  \
ZCOLORIN24(vx,Ropa[i],Gopa[i2],Bopa[i3],wi,wj,wk,image,zbuf); \
        }; 
