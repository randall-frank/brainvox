...
static void outputChar(int num, int width, int height,
  int xoff, int yoff, int advance, int data, FILE *fp)
{
  if (width == 0 || height == 0) {
    fprintf(fp,"#ifdef _WIN32\n");
    fprintf(fp,"/* XXX Work around Microsoft OpenGL 1.1 bug where glBitmap with\n");
    fprintf(fp,"   a height or width of zero does not advance the raster position\n");
    fprintf(fp,"   as specified by OpenGL. (Cosmo OpenGL does not have this bug.) */\n");
    fprintf(fp,"static const GLubyte ch%ddata[] = { 0x0 };\n", num);
    fprintf(fp,"static const BitmapCharRec ch%d = {", num);
    fprintf(fp,"%d,", 0);
    fprintf(fp,"%d,", 0);
    fprintf(fp,"%d,", xoff);
    fprintf(fp,"%d,", yoff);
    fprintf(fp,"%d,", advance);
    fprintf(fp,"ch%ddata", num);
    fprintf(fp,"};\n");
    fprintf(fp,"#else\n");
  }
  fprintf(fp,"static const BitmapCharRec ch%d = {", num);
...
void captureXFont(Display * dpy, Font font, char *xfont, char *name,FILE *fp)
{
  ...
  fprintf(fp,"#include \"igl_common.h\"\n\n");
    ...
    if (fontinfo->per_char != NULL) {
      ...
      if (charWidth == 0 || charHeight == 0) {
        if (charinfo->width != 0) {
           ...
           */
          outputChar(i, 0, 0, 0, 0, charinfo->width, 0,fp);
        }
        goto PossiblyDoGrab;
      }
    }
    ...
    if (numToGrab >= glyphsPerGrab || count == 1) {
      ...
      for (j = numToGrab - 1; j >= 0; j--) {
        ...
        if (PRINTABLE(grabList[j])) {
          fprintf(fp,"/* char: 0x%x '%c' */\n\n",
            grabList[j], grabList[j]);
        } else {
          fprintf(fp,"/* char: 0x%x */\n\n", grabList[j]);
        }
        ...
        if (!nullBitmap) {
          fprintf(fp,"static const GLubyte ch%ddata[] = {\n", grabList[j]);
          len = (charinfo->ascent + charinfo->descent) *
            ((charinfo->rbearing - charinfo->lbearing + 7) / 8);
          cnt = 0;
          while (cnt < len) {
            for (k = 0; k < 16 && cnt < len; k++, cnt++) {
              fprintf(fp,"0x%x,", bitmapData[cnt]);
            }
            fprintf(fp,"\n");
          }
          fprintf(fp,"};\n\n");
  ...
  fprintf(fp,"static const BitmapCharRec * const chars[] = {\n");
  ...
  fprintf(fp,"};\n\n");
  fprintf(fp,"const BitmapFontRec %s = {\n", name);
  fprintf(fp,"\"%s\",\n", xfont);
  fprintf(fp,"%d,\n", last - first + 1);
  fprintf(fp,"%d,\n", first);
  fprintf(fp,"chars\n");
...
int main(int argc, char **argv)
{
  ...
  if (argc != 4) {
    fprintf(stderr, "usage: capturexfont XFONT NAME outfile\n");
    exit(1);
  }
  ...
  fp = fopen(argv[3],"w");
  if (!fp) {
     fprintf(stderr, "capturexfont: uunable to open file: %s\n",argv[3]);
     exit(1);
  }
  captureXFont(dpy, font, argv[1], argv[2],fp);
  fclose(fp);
