...
#ifndef WIN32
#include	<X11/Xlib.h>	/* XXX do we need all this shit */
#include	<X11/Xutil.h>
#include	<X11/Xmd.h>
#include	<X11/cursorfont.h>

extern	Display *get_display(void);
extern  int	get_screen(void);
#endif
...
static int mhead;
static int PostTimeout;
static int HogTimeout;
static int MenuState;
static unsigned int StateTimeout;
...
#endif
static void timeout_happened(void);
static int history_allows(void);
...
static void update_radio_checks(MenuItem *);
...
extend_aFuncChain(aFuncChain* this, int (*newfunc)()) {
    ...
    if (newfunc) {
	newElement = CALLOC(sizeof *newElement);
	newElement->next = 0;
...
call_aFuncChain(aFuncChain* this, int ret) {
    aFuncElement	*who;
    typedef  int (*retintargint)(int);

    for (who = this->head; who; who = who->next)
	/* ret = ((int (*)(int))(who->func))(ret);*/
	ret = ((retintargint)(who->func))(ret);
    return ret;
}
...
typedef int (__cdecl *PFI)(int);

#endif


static void do_addpup(Menu* pup,char *str,va_list args) 
{
    MenuItem* it;
    int lastItemNo = 0;
    char *cp;
	...
	while (*cp && *cp != '|') {
		...
		*dp++ = *cp++;
		...
		switch (*++cp) {
		    ...
		    case 's': pup->retainLastY = 1; break;
		    ...
		    case 'd': isDisabled = 1; break;
		    case 'I': isChecked = 1; hasCheckbox = 1; break;
		    case 'i': isChecked = 0; hasCheckbox = 1; break;
		    case 'r':
		      radioGroup = strtol(++cp, &cp, 10);
		      hasCheckbox = 1;
		      cp--;
		      break;
		    case 'x':
		      returnValue = strtol(++cp, &cp, 10);
		      cp--;
		      break;
		    case '%': *dp++ = '%'; break;
		}
	...
	*/
	if (isTitle)
	    ...
	    hasCheckbox *= PUP_BOX;
	    if (isChecked) hasCheckbox |= PUP_CHECK;
	    if (hasCheckbox) set_check_item(it, hasCheckbox);
	    ...
	    set_enable_item(it, (isDisabled == 0));
	    if (hasUnderline)
...
{
#ifndef WIN32
    return (long) create_menu(get_display(), get_screen(), False);
#else
    return (long) create_menu(False);
#endif
}
...
long dopup_common(long menu,int button) 
{
    int ret;
#ifndef WIN32
    Menu* pup = (Menu*) menu;
    ...
    ret = popup_menu((Menu*) menu, button, x, y);
    XFlush(((Menu*)menu)->popupInfo->dpy);
#else
    POINT pnt;
    GetCursorPos(&pnt);
    ret = popup_menu((Menu*) menu, button, pnt.x, pnt.y);
#endif
    return ret;
}

/* used by the GUI elements */
/* default is RMB, in "new" mode, LMB */
long dopup2(long menu) 
{
    int btn = Button3;
    if (Checkmapping() == 0) btn = Button1;
    return(dopup_common(menu,btn));
}

/* used by the "content" windows (non-GUI) */
/* always RMB */
...
void setpup(long menu, long itemNo, unsigned long arg) 
{
    MenuItem* it;
...
void changepup(long menu, long itemNo, char *str, ... ) 
{
    va_list args;
    MenuItem* it;
    char *cp;
	...
	*/
	...
	while (*cp && *cp != '|') {
		...
		*dp++ = *cp++;
		...
		switch (*++cp) {
		    case 's': ((Menu *)menu)->retainLastY = 1; break;
		    case 'n': it->useMenuFunc = 0; /* Fall through to next case */
		    case 'f': it->itemFunc = va_arg(args, PFI); break;
		    case 'm': it->subMenu = va_arg(args, Menu*); break;
	            case 'd': set_enable_item(it, !it->enabled); break;
		    case 'I': hasCheckbox = 1; isChecked = 1; break;
		    case 'i': hasCheckbox = 1; isChecked = 0; break;
		    case 'r':
		      it->radioGroup = strtol(++cp, &cp, 10);
                      hasCheckbox = 1;
		      cp--;
		      break;
		    case 'x':
		      it->pickValue = strtol(++cp, &cp, 10);
		      cp--;
		      break;
		    case '%': *dp++ = '%'; break;
		}
	...
	if (dp == collectName) break;
	*dp = 0;

	hasCheckbox *= PUP_BOX;
	if (isChecked) hasCheckbox |= PUP_CHECK;
	set_check_item(it, hasCheckbox);

	if (it->label) free(it->label);
	it->label = STRDUP(collectName);
	it->len = strlen(collectName);
#ifdef WIN32
	it->trueWidth = it->width = 0;
	it->trueHeight = it->height = 0;
#else
	it->trueWidth = it->width = 
		strwidth_font(((Menu *)menu)->menuFont, collectName, it->len);
	it->trueHeight = it->height = Menu_itemTopInset
		+ ((Menu*)menu)->menuFont->finfo->ascent 
		+ ((Menu*)menu)->menuFont->finfo->descent
...
create_PopupInfo(Display* dpy, int screen) {
    ...
    ret->arrowCursor = XCreateFontCursor(dpy, XC_top_left_arrow);
    ret->overFirst = False;
...
#ifdef WIN32
    this->menuTitle = create_menu(True);
#else
...
#ifdef WIN32
static void HMENU_add(HMENU h, Menu *m)
{
   MenuItem* it;
...
/* start with a title */
   if (m->menuTitle) {
      it = m->menuTitle->head;
      AppendMenu(h,MF_GRAYED|MF_STRING,it->pickValue,it->label);
      AppendMenu(h,MF_GRAYED|MF_SEPARATOR,0,0);
   }

/* all the items */
   it = m->head;
   while(it) {
      int enab = MF_DISABLED|MF_GRAYED;
      if (it->enabled) enab = MF_ENABLED;

      if (!it->label) {
         AppendMenu(h,MF_DISABLED|MF_SEPARATOR,0,0);
      } else if (it->subMenu) {
         HMENU subm = CreatePopupMenu();
         HMENU_add(subm,it->subMenu);
         AppendMenu(h,MF_POPUP|MF_STRING|enab,(UINT_PTR)subm,it->label);
      } else if (it->type == MenuItem_hasCheckBox) {
         MENUITEMINFO mii = {0};
         AppendMenu(h,MF_STRING|enab,it->pickValue,it->label);
         mii.cbSize = sizeof(mii);
         mii.fMask = MIIM_DATA|MIIM_FTYPE|MIIM_STATE;
         mii.fType = MFT_STRING;
         if (it->radioGroup) mii.fType |= MFT_RADIOCHECK;
         mii.cch = strlen(it->label)+1;
         mii.dwTypeData = it->label;
         mii.dwItemData = (ULONG_PTR)it; /* pointer to self */
         mii.fState = MFS_GRAYED;
         if (it->enabled) mii.fState = MFS_ENABLED;
         if (it->checkState) mii.fState |= MFS_CHECKED;
         SetMenuItemInfo(h,it->pickValue,FALSE,&mii);
      } else {
         AppendMenu(h,MF_STRING|enab,it->pickValue,it->label);
      }
/*
it->checkState -> checked state
it->radioGroup >= 0 -> radio
it->type = MenuItem_hasCheckBox!!!
*/

/* For now, ignore:
 * retaining last Y
 * menu functions (and entry functions, etc)
 */
...
static MenuItem *find_menu_item(Menu *m,int val)
{
   MenuItem *it;
   it = m->head;
   while(it) {
      if (it->subMenu) {
         MenuItem *v = find_menu_item(it->subMenu,val);
         if (v) return(v);
      } else if (it->pickValue == val) {
         return(it);
      }
      it = it->nextItem;
   }
...
/* very different approach under Windows, using the native popup menus
 * dynamically create the menus, use and destroy them...
 */
   ...
   ret = TrackPopupMenu(menu,TPM_LEFTALIGN|TPM_TOPALIGN|TPM_NONOTIFY|
          TPM_RIGHTBUTTON|TPM_RETURNCMD,rx,ry,0,get_current_hwnd(),&r);

   if (ret > 0) {
      MenuItem *m = find_menu_item(this,ret);
      if (m) update_radio_checks(m);
   } else ret = -1;
...
popup_menu(Menu* this, int b, int rx, int ry) {
    ...
    if (!this->retainLastY) this->lastY = 0;
    realize_menu(this, rx, ry - this->lastY);
	...
	switch (ev.type) {
	  ...
	  case ButtonRelease:
		...
		if (result) {
		    currentMenu->lastY = result->y + (result->height*0.5);
                    update_radio_checks(result);
                }
...
create_item(Menu* m, int ix, char* label) {
    MenuItem* this = CALLOC(sizeof *this);
    this->parent = m;
...
set_check_item(MenuItem* this, int on) {
    ...
    this->checkState = 0;
    if (on & PUP_CHECK) this->checkState = 1;
    this->parent->layoutDone = False;
}

static void 
update_radio_checks(MenuItem *item) {
    if (item->type == MenuItem_normal) return;
    if (item->radioGroup >= 0) {
        MenuItem	*p;
        if (item->checkState) return;
	p = item->parent->head;
        while(p) {
	    if (p->radioGroup == item->radioGroup) {
	        p->checkState = 0;
            }
	    p = p->nextItem;
	}
    }
    item->checkState = 1 - item->checkState;
    item->parent->layoutDone = False;
    return;
}
...
void addtop_(long *pup,char *str,long *len,long *arg) 
{
    char *s = 0;
    if (*len > 0) {
        s = malloc(*len+1);
...
void change_(long *pup, long *item,char *str, long *len, long *arg)
{
    char *s = 0;
    if (*len > 0) {
        s = malloc(*len+1); 
...
void setpup_(long *pup, long *item, long *mode)
{
#ifdef DEBUG
    printf("setupu(%d,%d,%d)\n",*pup,*item,*mode);
#endif
